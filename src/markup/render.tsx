...
function renderMacro(node: MacroNode, key: number) {
  switch (node.name) {
    ...
    case "print":
      return <Print key={key} rawArgs={node.rawArgs} className={node.className} id={node.id} />;

    case "if":
      return <If key={key} branches={node.branches!} />;

    case "for":
      return <For key={key} rawArgs={node.rawArgs} children={node.children} className={node.className} id={node.id} />;
    ...
    case "button":
      return (
        <Button key={key} rawArgs={node.rawArgs} children={node.children} className={node.className} id={node.id} />
      );

    case "story-title":
      return <StoryTitle key={key} className={node.className} id={node.id} />;

    case "restart":
      return <Restart key={key} className={node.className} id={node.id} />;

    case "save":
      return <Save key={key} className={node.className} id={node.id} />;

    case "load":
      return <Load key={key} className={node.className} id={node.id} />;

    case "settings":
      return <SettingsButton key={key} className={node.className} id={node.id} />;
    ...
    default: {
      ...
      if (Component) {
        return (
          <Component key={key} rawArgs={node.rawArgs} className={node.className} id={node.id}>
...
export function renderNodes(nodes: ASTNode[]): preact.ComponentChildren {
  return nodes.map((node, i) => {
    switch (node.type) {
      ...
      case "link":
        return (
          <PassageLink key={i} target={node.target} className={node.className} id={node.id}>
            {node.display}
          </PassageLink>
        );

      case "variable":
        return <VarDisplay key={i} name={node.name} scope={node.scope} className={node.className} id={node.id} />;
