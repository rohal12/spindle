...
describe("tokenize", () => {
  ...
  describe("links", () => {
    it("parses [[passage]] as plain link", () => {
      ...
      expect(tokens).toEqual([
        { type: "link", display: "Garden", target: "Garden", start: 0, end: 10 },
      ]);
    });
    ...
    it("trims whitespace in link parts", () => {
      ...
      expect(tokens).toEqual([
        { type: "link", display: "display", target: "target", start: 0, end: 26 },
      ]);
    });
    ...
    it("handles multiple links with text between", () => {
      ...
      expect(tokens[0]).toEqual({ type: "text", value: "Go ", start: 0, end: 3 });
      expect(tokens[1]).toEqual({ type: "link", display: "Left", target: "Left", start: 3, end: 11 });
      expect(tokens[2]).toEqual({ type: "text", value: " or ", start: 11, end: 15 });
      expect(tokens[3]).toEqual({ type: "link", display: "Right", target: "Right", start: 15, end: 24 });
    });
    ...
    it("handles all four syntaxes in the same content", () => {
      ...
      expect(links.map((l) => ({ d: (l as any).display, t: (l as any).target }))).toEqual([
        { d: "plain", t: "plain" },
        { d: "d", t: "t" },
        { d: "d", t: "t" },
        { d: "d", t: "t" },
  ...
  describe("variables", () => {
    it("parses {$var} as variable token", () => {
      ...
      expect(tokens).toEqual([
        { type: "variable", name: "health", scope: "variable", start: 0, end: 9 },
      ]);
    });
    ...
    it("parses {_temp} as temporary variable token", () => {
      ...
      expect(tokens).toEqual([
        { type: "variable", name: "count", scope: "temporary", start: 0, end: 8 },
      ]);
    });
    ...
    it("handles variable in text", () => {
      ...
      expect(tokens[0]).toEqual({ type: "text", value: "Health: ", start: 0, end: 8 });
      expect(tokens[1]).toEqual({ type: "variable", name: "health", scope: "variable", start: 8, end: 17 });
      expect(tokens[2]).toEqual({ type: "text", value: " points", start: 17, end: 24 });
    });
  ...
  describe("macros", () => {
    it("parses {set $x = 5} as macro token", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "set", rawArgs: "$x = 5", isClose: false, start: 0, end: 12 },
      ]);
    });
    ...
    it("parses {/if} as closing macro token", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "if", rawArgs: "", isClose: true, start: 0, end: 5 },
      ]);
    });
    ...
    it("parses {else} as a macro", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "else", rawArgs: "", isClose: false, start: 0, end: 6 },
      ]);
    });
    ...
    it("parses {elseif $x > 3} with args", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "elseif", rawArgs: "$x > 3", isClose: false, start: 0, end: 15 },
      ]);
    });
    ...
    it("parses {print $health * 2}", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "print", rawArgs: "$health * 2", isClose: false, start: 0, end: 19 },
      ]);
    });
    ...
    it("parses {for $item, $i of $list}", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "for", rawArgs: "$item, $i of $list", isClose: false, start: 0, end: 24 },
      ]);
    });
  ...
  describe("CSS class syntax", () => {
    it("parses {.class $var} as variable with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "variable", name: "name", scope: "variable", className: "hero-name", start: 0, end: 18 },
      ]);
    });
    ...
    it("parses {.class _temp} as temporary variable with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "variable", name: "count", scope: "temporary", className: "muted", start: 0, end: 15 },
      ]);
    });
    ...
    it("parses multiple classes on variable: {.foo.bar $var}", () => {
      ...
      expect(tokens).toEqual([
        { type: "variable", name: "name", scope: "variable", className: "foo bar", start: 0, end: 16 },
      ]);
    });
    ...
    it("parses {.class macroName args} as macro with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "button", rawArgs: "$health -= 10", isClose: false, className: "danger", start: 0, end: 30 },
      ]);
    });
    ...
    it("parses multiple classes on macro: {.danger.large button ...}", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "button", rawArgs: "$health -= 10", isClose: false, className: "danger large", start: 0, end: 36 },
      ]);
    });
    ...
    it("parses {.class if $cond} as macro with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "if", rawArgs: "$health < 50", isClose: false, className: "highlight", start: 0, end: 28 },
      ]);
    });
    ...
    it("parses {.class print expr} as macro with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "print", rawArgs: "$visited_rooms", isClose: false, className: "muted", start: 0, end: 29 },
      ]);
    });
    ...
    it("closing tags do not take classes", () => {
      ...
      expect(tokens).toEqual([
        { type: "macro", name: "button", rawArgs: "", isClose: true, start: 0, end: 9 },
      ]);
    });
    ...
    it("parses [[.class link]] with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "link", display: "Open the door", target: "Hallway", className: "fancy", start: 0, end: 32 },
      ]);
    });
    ...
    it("parses [[.class.class2 link]] with multiple classes", () => {
      ...
      expect(tokens).toEqual([
        { type: "link", display: "Go", target: "Start", className: "fancy bold", start: 0, end: 24 },
      ]);
    });
    ...
    it("parses [[.class plain]] plain link with className", () => {
      ...
      expect(tokens).toEqual([
        { type: "link", display: "Garden", target: "Garden", className: "fancy", start: 0, end: 17 },
      ]);
    });

    it("tokens without classes have no className property", () => {
      const tokens = tokenize("{$name}");
      expect(tokens[0]).toEqual({ type: "variable", name: "name", scope: "variable", start: 0, end: 7 });
